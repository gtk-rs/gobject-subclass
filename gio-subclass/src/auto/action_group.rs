...
(gptr: *mut gio_ffi::GActionGroup, action_name: *const c_char, enabled: *mut gboolean, parameter_type: *mut *const glib_ffi::GVariantType, state_type: *mut *const glib_ffi::GVariantType, state_hint: *mut *mut glib_ffi::GVariant, state: *mut *mut glib_ffi::GVariant) -> gboolean
{
    ...
    std::ptr::write(enabled, rs_enabled.to_glib());
    std::ptr::write(parameter_type, rs_parameter_type.to_glib_full());
    std::ptr::write(state_type, rs_state_type.to_glib_full());
    std::ptr::write(state_hint, rs_state_hint.to_glib_full());
    std::ptr::write(state, rs_state.to_glib_full());
    rs_ret.to_glib()
}
...
unsafe extern "C" fn action_group_init<T: ObjectType>(
    ...
    let actiongroup_iface = &mut *(iface as *mut gio_ffi::GActionGroupInterface);
